(function(){
  ...
  function openUserModal(id){
    ...
    if(!user) return;
    lockPageScroll();
    var editButton=(typeof isAdminUser==='function'&&isAdminUser())?'<button class="userModalEditCog" id="userModalEditCog" type="button" aria-label="Edit Profile" title="Edit Profile">⚙</button>':'';
    modalRoot.innerHTML='<div class="modalCard userModalCard" role="dialog" aria-modal="true" aria-label="User profile"><div class="userModalStickyHeader"><div class="userModalHeader"><span class="avatarText big">'+safe(initials(user))+'</span><h2>'+safe(user.name||user.nickname)+'</h2>'+editButton+'<button class="close userModalClose" id="closeUserModal" type="button">×</button></div><div class="userModalTabs"><button data-user-modal-section="personal">Personal details</button><button data-user-modal-section="employment">Employment</button><button data-user-modal-section="shifts">Shifts</button><button data-user-modal-section="training">Training</button><button data-user-modal-section="availability">Availability</button></div></div><div id="userModalDetail" class="panel userModalDetail">'+detail(user,'personal')+'</div></div>';
  ...
  function loadEmploymentPatch(){
    if(window.__employmentDocumentProfilePatchV1||document.getElementById('employmentDocumentProfileScript')) return;
    var script=document.createElement('script');
    script.id='employmentDocumentProfileScript';
    script.src='src/users/employment-document-profile.js?v=20260618-1';
    script.defer=true;
  ...
  if(typeof bind==='function'&&!bind.__userButtonsModalStyle){
    var oldBind=bind;
    bind=function(){oldBind();loadEmploymentPatch();var search=document.getElementById('centralPeopleSearch');document.querySelectorAll('[data-new-user]').forEach(function(button){button.textContent='Add User';button.setAttribute('aria-label','Add User');});if(search){search.placeholder='Search Staff..';search.oninput=window.drawCentralPeopleList;window.drawCentralPeopleList();}};
    bind.__userButtonsModalStyle=true;
  }
  setTimeout(function(){loadEmploymentPatch();var search=document.getElementById('centralPeopleSearch');if(search)search.placeholder='Search Staff..';document.querySelectorAll('[data-new-user]').forEach(function(button){button.textContent='Add User';button.setAttribute('aria-label','Add User');});if(document.getElementById('centralPeopleList'))window.drawCentralPeopleList();},0);
})();