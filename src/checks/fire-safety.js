(function(){
  ...
  function readEvidence(file,done){if(!file){done(null);return;}if(window.ComplianceFirebase&&typeof window.ComplianceFirebase.uploadFile==='function'){window.ComplianceFirebase.uploadFile(file,{kind:'check',folder:'check-evidence'}).then(done).catch(function(error){alert(error&&error.message||'Upload failed. Check connection.');done(null);});return;}var r=new FileReader();r.onload=function(){done({fileName:file.name||'Evidence photo',fileType:file.type||'',fileData:r.result||'',uploadedAt:new Date().toISOString(),storageMode:'local'});};r.readAsDataURL(file);}
