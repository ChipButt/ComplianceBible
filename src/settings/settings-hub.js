...
(function coreSettingsHub() {
  ...
  function quickUserControls() {
    var users = visibleUsers();
    return '<div class="coreQuickUsers"><label class="coreQuickAll"><input type="checkbox" data-select-all-users><span>All users</span></label><label class="coreQuickSelect"><span>Add by work area</span><select data-select-area-users><option value="">Select work area</option>' + areas().map(function (area) { return '<option value="' + h(area) + '">' + h(area) + '</option>'; }).join('') + '</select></label><label class="coreQuickSelect"><span>Add individual user</span><select data-select-individual-user><option value="">Select user</option>' + users.map(function (user) { return '<option value="' + h(user.id) + '">' + h(user.name || user.nickname || user.id) + '</option>'; }).join('') + '</select></label></div>';
  }
  ...
  function bindGroupSelectors(form) {
    ...
    var individual = form.querySelector('[data-select-individual-user]');
    if (individual) individual.onchange = function () {
      var userId = individual.value;
      if (!userId) return;
      var input = form.querySelector('[name="user__' + cssEscape(userId) + '"]');
      if (input && !input.disabled) input.checked = true;
      individual.value = '';
    };
  }
