...
(function completeSettingsHubCleanV1(){
  ...
  function areas(){return'<section class="settingsBlock compactSettingsBlock"><h2>Areas</h2><p class="muted">Used by checks, user profiles and rota sections.</p>'+compactRows(state.areas||[],'delete-area','Delete area')+'<form id="areaForm" class="settingsCompactForm"><input name="area" placeholder="New area" required><button class="primary">Add area</button></form></section>';}
  function rotaSetup(){const sections=(state.rotaSettings&&state.rotaSettings.sections)||[];return'<section class="settingsBlock compactSettingsBlock"><h2>Rota setup</h2><p class="muted">Sections available when building rota shifts.</p>'+compactRows(sections,'delete-rota-section','Delete rota section')+'<form id="rotaSectionForm" class="settingsCompactForm"><input name="section" placeholder="New rota section" required><button class="primary">Add rota section</button></form></section>';}
  ...
  function bindSettingsModalForms(){
    ...
    document.querySelectorAll('#modal.settingsModalOpen [data-delete-rota-section]').forEach(button=>button.onclick=()=>{state.rotaSettings=state.rotaSettings||{sections:[]};state.rotaSettings.sections=(state.rotaSettings.sections||[]).filter(section=>section!==button.dataset.deleteRotaSection);saveSafe();finishSettingsSave();});
