...
(function coreSettingsHub() {
  ...
  function sectionBody(id) {
    ...
    if (id === 'issues') return '<h3>Issues</h3>' + summary(['issues.view', 'issues.manage', 'issues.notify']) + '<h3>Inspection</h3>' + summary(['inspection.view', 'inspection.export']);
  ...
  function usersSection() {
    return '<div class="coreUserPreview">' + (state.users || []).map(function (user) {
      ...
      return '<div class="coreUserRow"><span class="avatarText">' + h(initials(user)) + '</span><div><strong>' + h(user.name || user.nickname || 'User') + '</strong><em>' + h(user.jobArea || user.area || user.role || '') + '</em></div><small>' + (tabs.length ? 'Opens: ' + tabs.join(', ') : 'No profile tab access') + '</small></div>';
